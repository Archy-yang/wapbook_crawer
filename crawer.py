...
def getter():
    ...
    for i in doc('select[name="pageselect"]>option').items():
        ...
        items = pageDoc('div.directoryArea').items()
        items.next()
        menu = items.next()
        for a_tag in menu('a').items():
            ...
            title = page('span.title').html().encode('utf-8')
            readpage = page('p.Readpage').next().html().encode('utf-8')
            content = page('div#chaptercontent').html().encode('utf-8')
